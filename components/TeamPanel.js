        ...
        {!result ? (
          <>
            ...
            <input
              ...
              placeholder="Ex: Ethiquê Perícias"
          ...
          <div className="fade">
            <div className="card-surface" style={{ marginBottom: '1rem' }}>
              <span className="label" style={{ display: 'block', marginBottom: 6 }}>Projeto criado com sucesso</span>
              <p style={{ fontWeight: 500, fontSize: 14, marginBottom: 4 }}>{result.company}</p>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Olá! Segue o link para preencher o questionário de marca da Melo Creative:\n\n${result.link}`)}`}
                target="_blank"
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
      <div className="card fade" style={{ width: '100%', maxWidth: 400 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
...
export default function TeamPanel() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      {showModal && <NewProjectModal onClose={() => setShowModal(false)} onRefresh={refresh} />}
      {deleteTarget && <DeleteConfirmModal item={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={confirmDelete} deleting={deleting} />}
        ...
        <div style={{ width: 230, borderRight: '1px solid var(--border)', padding: '1rem', flexShrink: 0, overflowY: 'auto' }}>
          ...
          {!loading && (index || []).slice().reverse().map(item => {
            ...
            return (
              <div
                key={item.id}
                className={`sidebar-item${sel?.id === item.id ? ' active' : ''}`}
                style={{ opacity: clickable ? 1 : 0.55 }}
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
                  <p
                    onClick={() => clickable ? select(item) : null}
                    style={{ fontSize: 13, fontWeight: sel?.id === item.id ? 500 : 400, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1, cursor: clickable ? 'pointer' : 'default', marginBottom: 3 }}
                  >
                    {item.company}
                  </p>
                  <button
                    onClick={e => { e.stopPropagation(); setDeleteTarget(item) }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', fontSize: 14, padding: '2px 4px', flexShrink: 0, lineHeight: 1 }}
            ...
            <div className="fade">
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.25rem', gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {analysis && (
                    <button className="btn" onClick={() => downloadPDF(analysis, responses?.nome || sel.company)}>↓ PDF</button>
                  )}
              ...
              {sel.status === 'aguardando' && (
                <div className="alert alert-warn" style={{ marginBottom: '1rem' }}>
                  O cliente ainda não respondeu. Compartilhe o link de acesso com ele.
                </div>
              )}
