...
export default function TeamPanel() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Atualiza lista local imediatamente sem esperar refresh
      setIndex(prev => prev.filter(i => i.id !== deleteTarget.id))
      // Se era o selecionado, limpa o painel direito
      if (sel?.id === deleteTarget.id) {
        setSel(null)
        setResponses(null)
        setAnalysis(null)
      }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <nav className="nav">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button className="btn btn-sm btn-ghost" onClick={refresh} title="Atualizar lista">↻ Atualizar</button>
